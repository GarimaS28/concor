...
@Component({
  selector: 'app-gate-in-pass-stuffing',
  standalone: true,
  imports: [SharedModule, FormsModule,],
  templateUrl: './gate-in-pass-stuffing.component.html',
  styleUrls: ['./gate-in-pass-stuffing.component.scss'],
  animations: [
    trigger('fadeIn', [
      transition(':enter', [
        style({ opacity: 0 }),
...
export class GateInPassStuffingOutwardComponent implements OnInit {
 ...
 constructor(private helpService: HelpService,private fb: FormBuilder, private router: Router,private gateservice:GateserviceService, private gatepasstore: GatepassstoreserviceService) {
  ...
  this.form.get('GTInCurrDate')?.setValue(todayFormatted);
...
this.form.get('GTInIssueTm')?.setValue(formattedNow);
this.form.get('GTInDtTime')?.setValue(formattedNow);


  this.setEndOfDay();
this.form.get('GTInVldUpTo')?.setValue(this.endOfDay);
