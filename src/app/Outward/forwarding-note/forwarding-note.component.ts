import { Component,  ElementRef,  OnInit, ViewChild } from '@angular/core';
    ...
    @Component({
      selector: 'app-forwarding-note',
      standalone: true,
      imports: [SharedModule, FormsModule,],
      templateUrl: './forwarding-note.component.html',
      styleUrls: ['./forwarding-note.component.scss'],
      animations: [
        trigger('fadeIn', [
          transition(':enter', [
            style({ opacity: 0 }),
    ...
    export class ForwardingNoteComponent implements OnInit {

      srltab = Array.from({ length: 1 }, () => ({ Srlno: '' }));
      @ViewChild('scrollContainer') scrollContainer!: ElementRef;

      cmdttab = Array.from({ length: 2 }, () => ({ CmdtCode: '', CmdtDesc: '', CmdtPkgs: '', CmdtWght: '', CmdtHsnCode: '' }));
      @ViewChild('scrollContainerr') scrollContainerr!: ElementRef;
      ...
      // Set today's date in yyyy-MM-dd format for input[type="date"]
          ...
          this.form.get('FwdNtDate')?.setValue(todayFormatted);
        ...
        this.form.get('FwdNtSelngTime')?.setValue(formattedNow);
  ...
  return `${dd}-${mm}-${yyyy.slice(2)}`;
    ...
    onSubmit() {
      ...
      const finalPayload = {
        ...
        // ISSUE_TIME: "19-05-25 23:59",  // e.g. 'dd-MM-yy HH:mm'
        // RFND_FLAG: "E",
        ...
        // VLDT_UPTO: "19-05-25 23:59",
        // WGHT: formValues.GOIPvhclwght,
        // ICOUNT: "0",
        // CHK_FLAG: "I",
        // OPT_SLCT: "S",
        // CNTR_CC: "", // Optional
        // GATE_INOUTTIME: "19-05-25 23:59", // same as ISSUE_TIME
        // RFND_NUMB: "",
        // PERMIT_NUMB: "",
        // JV_LOGIN: "*",
    ...
    addRow() {
      this.srltab.push({ Srlno: '' });
      setTimeout(() => {
        this.scrollToBottom();
      }, 0);
    }
    removeRow(index: number): void {
      if (this.srltab.length > 1) {
        this.srltab.splice(index, 1);
      }
    }

    addRowC() {
      this.cmdttab.push({ CmdtCode: '', CmdtDesc: '', CmdtPkgs: '', CmdtWght: '', CmdtHsnCode: '' });
      setTimeout(() => {
        this.scrollToBottomC();
      }, 0);
    }
    removeRowC(index: number): void {
      if (this.cmdttab.length > 1) {
        this.cmdttab.splice(index, 1);
      }
    }
    ...
    private scrollToBottomC() {
      try {
        const container = this.scrollContainerr.nativeElement;
        container.scrollTop = container.scrollHeight;
      } catch (err) {
        console.error('Scroll error:', err);
      }
