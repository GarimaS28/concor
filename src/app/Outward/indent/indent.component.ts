...
@Component({
  selector: 'app-indent',
  standalone: true,
  imports: [SharedModule, FormsModule, LoaderComponent],
...
export class IndentComponent implements OnInit {
    ...
    this.form = this.fb.group(
      {
        ...
        ALOTPaidby: ['C', Validators.required],
  ...
  ngOnInit(): void {
    this.handleServiceModeChange();

    this.form.get('ALOTsrvcMd')?.valueChanges.subscribe((value: string) => {
      this.updateFieldsBasedOnService(value);
    });

    this.form.get('ALOTtype')?.valueChanges.subscribe((value: string) => {
      if (value === 'N' || value === 'P') {
        this.calculateContainerCharges(value);
      }
    });
    ...
    this.form = this.fb.group({
      ALOTPaidby: new FormControl({ value: 'C', disabled: true }), // selected & disabled
      ALOTtype: [''],
      ALOTamount: ['']
    });
    ...
    this.form
      ...
      .subscribe((stationCode: string) => {
        if (stationCode) {
          this.helpService.getConsigneeHelpData(stationCode).subscribe({
            next: (data) => {
              console.log('Consignee Data:', data);
              // No need to assign here, handled by customerData$
            },
            error: (err) => {
              console.error('Error fetching consignee help data:', err);
            }
          });
        }
      });
    ...
    this.helpService.TrmnlCodeTo$.subscribe((data) => {
      if (data?.result) {
        ...
        this.filteredStationToList = [...this.TrmnlDataTO]; // Initialize with full list
        console.log('Terminal To Data received:', this.TrmnlDataTO);
      }
    });
    ...
    //      console.log('Customer ID Help Data:', this.consignerIdHelpData);
    //    }
    //  });
    ...
    //      console.log('Customer ID Help Data:', this.consigneeIdHelpData);
    //    }
    //  });
  ...
  helpDataMap: Map<string, any[]> = new Map(); // local cache
  showDropdown = false;

  filterCmdtList(value: string): void {
    this.filteredCmdtList = this.helpData.filter(
      (item) => item.p_CmdtCode.toLowerCase().includes(value.toLowerCase()) || item.p_CmdtDesc.toLowerCase().includes(value.toLowerCase())
    );
    this.showDropdown = true;
  }

  selectCmdtCode(item: any): void {
    this.selectedCmdtCode = item.p_CmdtCode;
    this.selectedCmdtDesc = item.p_CmdtDesc;
    this.selectedCmdtType = item.p_CmdtType;

    this.form.get('ALOTcmdtCode')?.setValue(item.p_CmdtCode);
    this.form.get('ALOTcmdtDesc')?.setValue(item.p_CmdtDesc);

    this.form.get('ALOTcmdtCode')?.markAsTouched();
    this.form.get('ALOTcmdtDesc')?.markAsTouched();
  ...
  filterStationList(search: string) {
    ...
    this.filteredStationList = this.TrmnlData.filter((station) => station.p_TrmnlCode?.toLowerCase().includes(search.toLowerCase()));
  }
  ...
  validateStationFromInput() {
    ...
    const isValid = this.TrmnlData.some((station) => station.p_TrmnCode === enteredCode);
  ...
  filterStationToList(search: string) {
    ...
    this.filteredStationToList = this.TrmnlDataTO.filter((station) => station.p_TrmnCode.toLowerCase().includes(search.toLowerCase()));
  }

  selectStationToCode(station: any) {
    this.selectedStationToCode = station.p_TrmnCode;
    this.form.get('ALOTsttnto')?.setValue(station.p_TrmnCode);
    this.showStationToDropdown = false;

    // 🚨 Clear dependent address fields before fetching new ones
    this.form.get('ALOTcnseCode')?.reset();
    this.form.get('ALOTcnseName')?.reset();
  ...
  onStationToInputChange(event: any) {
    ...
    this.filterStationToList(value);
    this.form.get('ALOTsttnto')?.setValue(value);
  }
  ...
  validateStationToInput() {
    ...
    const isValid = this.TrmnlDataTO.some((station) => station.p_TrmnCode === enteredCode);

    if (!isValid) {
      // Clear the value
      this.selectedStationToCode = '';
      this.form.get('ALOTsttnto')?.setValue('');

      // Force validation class to show red by touching the field
      this.form.get('ALOTsttnto')?.markAsTouched();
      this.form.get('ALOTsttnto')?.markAsDirty();
    }
  }
  ...
  // filterContainerType(search: string, index: number) {
  //   const query = search.toLowerCase();
  //   this.filteredContainerTypes = this.containerHelpData.filter(item =>
  //     item.p_CntrType.toLowerCase().includes(query)
  //   );
  //   this.showContainerHelp[index] = true;
  // }

  // selectContainerType(item: any, index: number) {
  //   const row = this.containerArray.at(index);
  //   row.patchValue({
  //     ALOTctnrType: item.p_CntrType,
  //     ALOTctnrSize: item.p_CntrSize
  //   });

  //   this.showContainerHelp[index] = false;
  //   this.checkContainerRowCompletion(index);
  // }
  ...
  filterConsignerIdList(search: string) {
    this.filteredConsignerIdList = this.consignerIdHelpData.filter(
      (item) => item.p_CustID.toLowerCase().includes(search.toLowerCase()) || item.p_CustName.toLowerCase().includes(search.toLowerCase())
    );
  }
  ...
  filterConsigneeIdList(search: string) {
    this.filteredConsigneeIdList = this.consigneeIdHelpData.filter(
      (item) => item.p_CustID.toLowerCase().includes(search.toLowerCase()) || item.p_CustName.toLowerCase().includes(search.toLowerCase())
    );
  }
  ...
  filterConsignerList(value: string) {
    this.filteredConsignerList = this.consignerHelpData.filter(
      (item) => item.p_CustCode.toLowerCase().includes(value.toLowerCase()) || item.p_CustName.toLowerCase().includes(value.toLowerCase())
    );
  }
  ...
  filterConsigneeList(value: string) {
    this.filteredConsigneeList = this.consigneeHelpData.filter(
      (item) => item.p_CustCode.toLowerCase().includes(value.toLowerCase()) || item.p_CustName.toLowerCase().includes(value.toLowerCase())
    );
  }
  ...
  //containerArray = this.allotForm.get('ALOTcontainerArray') as FormArray;
  ...
  calculateContainerCharges(paidBy: string) {
    ...
    const payload = {
      ...
      p_RateType: this.form.get('ALOTrateType')?.value,
      p_CmdtCode: this.form.get('ALOTcmdtCode')?.value,
      p_CmdtCodeType: this.selectedCmdtType,
      p_ActyCode: 'REGN',
      p_JvCode: '*',
    ...
    this.gateservice.getContainerCharges(payload).subscribe({
      next: (res) => {
        if (res?.Result && Array.isArray(res.Result)) {
          this.totalAmount = res.Result.reduce((sum: number, item: any) => sum + Number(item.p_TotalChrg || 0), 0);
          this.form.get('ALOTamount')?.setValue(String(this.totalAmount)); // ✅ convert to string here
        } else {
          this.totalAmount = 0;
          this.form.get('ALOTamount')?.setValue('0'); // ✅ ensure default string
        }
      },
      error: (err) => {
        console.error('Charge calculation error:', err);
        this.totalAmount = 0;
        this.form.get('ALOTamount')?.setValue('0'); // ✅ fallback string
      }
    });
  }
  ...
  handleServiceModeChange() {
    this.form.get('ALOTsrvcMd')?.valueChanges.subscribe((value: string) => {
      const reqdDateControl = this.form.get('ALOTreqdDt');
      const dlvrDateControl = this.form.get('ALOTdlvrDt');

      switch (value) {
        case 'DD':
          reqdDateControl?.enable();
          reqdDateControl?.setValidators(Validators.required);
          dlvrDateControl?.enable();
          dlvrDateControl?.setValidators(Validators.required);
          break;

        case 'TD':
        case 'CD':
          reqdDateControl?.disable();
          reqdDateControl?.clearValidators();
          reqdDateControl?.reset();
        ...
        case 'DT':
        case 'DC':
          dlvrDateControl?.disable();
          dlvrDateControl?.clearValidators();
          dlvrDateControl?.reset();
      ...
      reqdDateControl?.updateValueAndValidity();
      dlvrDateControl?.updateValueAndValidity();
    });
  }

  updateFieldsBasedOnService(mode: string): void {
    const cnsrDist = this.form.get('ALOTcnsrDist');
    const cnsrPikup = this.form.get('ALOTcnsrPikup');
    const cnseDist = this.form.get('ALOTcnseDist');
    const cnseDlvr = this.form.get('ALOTcnseDlvr');

    // First disable and clear all
    [cnsrDist, cnsrPikup, cnseDist, cnseDlvr].forEach((control) => {
      control?.disable();
    ...
    // Enable and set required as per selected service mode
    if (mode === 'DD') {
      [cnsrDist, cnsrPikup, cnseDist, cnseDlvr].forEach((control) => {
        control?.enable();
    ...
    } else if (mode === 'DT' || mode === 'DC') {
      [cnsrDist, cnsrPikup].forEach((control) => {
        control?.enable();
        control?.setValidators(Validators.required);
        control?.updateValueAndValidity();
      });
    } else if (mode === 'TD' || mode === 'CD') {
      [cnseDist, cnseDlvr].forEach((control) => {
        control?.enable();
        control?.setValidators(Validators.required);
        control?.updateValueAndValidity();
      });
  ...
  // Mock list (replace with actual help list)

  filterContainerType(index: number) {
    const inputValue = this.containerArray.at(index).get('ALOTctnrType')?.value || '';
    this.filteredContainerTypes = this.containerHelpData.filter((item) => item.p_CntrType.toLowerCase().includes(inputValue.toLowerCase()));
  }
  ...
  onClickOutside(event: MouseEvent): void {
    ...
    // Check if click is outside `.position-relative`
    if (!target.closest('.position-relative')) {
      this.showSubtaskMenu = false;
    }

    // Check if click is NOT inside help dropdown or input
  ...
  showConsignerpickupDropdown: boolean = false;

  filterConsignerPickupFromEvent(event: Event) {
    const input = event.target as HTMLInputElement;
    const searchValue = input.value;

    this.form.get('ALOTcnsrPikup')?.setValue(searchValue); // Ensure Reactive Form is updated
    this.filterConsignerPickupList(searchValue);
    this.showConsignerpickupDropdown = true;
  }

  filterConsignerPickupList(search: string) {
    this.filteredConsignerPickList = this.consignerPickData.filter(
      (item) =>
        item.p_PickDlvrName.toLowerCase().includes(search.toLowerCase()) || item.p_PickDlvrDist.toLowerCase().includes(search.toLowerCase())
    );
  }

  consignerDropdownpickupTimeout: any;
  clearConsignerpickupDropdownTimeout() {
    clearTimeout(this.consignerDropdownpickupTimeout);
  }

  selectConsignerpickup(item: any) {
    this.selectedConsignerPick = item.p_PickDlvrName;
    this.form.get('ALOTcnsrPikup')?.setValue(item.p_PickDlvrName);

    this.onConsignerpickupChange(item.p_PickDlvrName);

    this.showConsignerpickupDropdown = false;
  }

  hideConsignerpickupDropdownWithDelay() {
    this.consignerDropdownpickupTimeout = setTimeout(() => {
      this.showConsignerpickupDropdown = false;
    }, 200);
  }

  onConsignerpickupChange(consignerpickup: string) {
    const stationCode = this.form.get('ALOTsttnfrm')?.value;

    if (!stationCode) {
      console.warn('❌ Missing  stationCode for pickup Help');
      return;
    }

    // Reset fields first
    this.form.get('ALOTcnsrPikup')?.reset();
    this.form.get('ALOTcnsrDist')?.reset();

    this.selectedConsignerPick = consignerpickup;
    ...
    this.helpService.getpickupDlvrData(stationCode).subscribe({
      next: (data) => {
        if (data?.result) {
          ...
          this.filteredConsignerPickList = this.consignerPickData.filter((item) =>
            item.p_PickDlvrName.toLowerCase().includes(consignerpickup.toLowerCase())
          );

          const found = this.consignerPickData.find((item) => item.p_PickDlvrName === consignerpickup);

          this.selectedCnsrdist = found?.p_PickDlvrDist || '';

          if (found) {
            this.form.get('ALOTcnsrDist')?.setValue(found.p_PickDlvrDist);
          } else {
            console.warn('⚠️ No consigner pick up match found');
          }
        }
      },
      error: (err) => {
        console.error('❌ Error fetching CustomerIdpickupData:', err);
      }
    });
  }

  showConsigneepickupDropdown: boolean = false;

  filterConsigneePickupFromEvent(event: Event) {
    const input = event.target as HTMLInputElement;
    const searchValue = input.value;

    this.form.get('ALOTcnseDlvr')?.setValue(searchValue); // Ensure Reactive Form is updated
    this.filterConsigneePickupList(searchValue);
    this.showConsigneepickupDropdown = true;
  }

  filterConsigneePickupList(search: string) {
    this.filteredConsigneePickList = this.consigneePickData.filter(
      (item) =>
        item.p_PickDlvrName.toLowerCase().includes(search.toLowerCase()) || item.p_PickDlvrDist.toLowerCase().includes(search.toLowerCase())
    );
  }

  consigneeDropdownpickupTimeout: any;
  clearConsigneepickupDropdownTimeout() {
    clearTimeout(this.consigneeDropdownpickupTimeout);
  }

  selectConsigneepickup(item: any) {
    this.selectedConsigneeDlvr = item.p_PickDlvrName;
    this.form.get('ALOTcnseDlvr')?.setValue(item.p_PickDlvrName);

    this.onConsigneepickupChange(item.p_PickDlvrName);

    this.showConsigneepickupDropdown = false;
  }

  hideConsigneepickupDropdownWithDelay() {
    this.consigneeDropdownpickupTimeout = setTimeout(() => {
      this.showConsigneepickupDropdown = false;
    }, 200);
  }

  onConsigneepickupChange(consigneepickup: string) {
    const stationCode = this.form.get('ALOTsttnto')?.value;
    ...
    // Reset fields first
    this.form.get('ALOTcnseDlvr')?.reset();
    this.form.get('ALOTcnseDist')?.reset();

    this.selectedConsigneeDlvr = consigneepickup;
    ...
    this.helpService.getpickupDlvrData(stationCode).subscribe({
      next: (data) => {
        if (data?.result) {
          ...
          this.filteredConsigneePickList = this.consigneePickData.filter((item) =>
            item.p_PickDlvrName.toLowerCase().includes(consigneepickup.toLowerCase())
          );

          const found = this.consigneePickData.find((item) => item.p_PickDlvrName === consigneepickup);

          this.selectedCgnedist = found?.p_PickDlvrDist || '';

          if (found) {
            this.form.get('ALOTcnseDist')?.setValue(found.p_PickDlvrDist);
          } else {
            console.warn('⚠️ No consigner pick up match found');
          }
        }
  ...
  // isFormValidAndEnabledFieldsFilled(): boolean {
  //   if (!this.form) return false;

  //   // Check if all enabled controls are non-empty and valid
  //   const controls = this.form.controls;
  //   for (const key in controls) {
  //     const control = controls[key];
  //     if (control.enabled && (control.invalid || control.value === null || control.value === '')) {
  //       return false;
  //     }
  //   }
  ...
  // // Flag to indicate Enter was pressed (to delay blur closing)
  // private enterPressedForField: { [field: string]: boolean } = {};
  ...
  // ) {
  //   event.preventDefault();
  //   const idx = this.highlightedIndices[fieldName] ?? -1;
  //   if (idx >= 0 && idx < filteredList.length) {
  //     selectFn(filteredList[idx]);
  //   }
  //   // Mark that Enter was pressed so blur handler knows to not close immediately
  //   this.enterPressedForField[fieldName] = true;
  // }

  // On blur, delay closing if Enter was just pressed
